...
function gateKeeper(req, res, next) {
  ...
  if (req.get("x-username-and-password")) {
   const params = queryString.parse(req.get("x-username-and-password"));
    console.log(params.user, USERS.find((entry) => entry.userName === params.user).userName);
    if (params.user === USERS.find((entry) => entry.userName === params.user).userName && params.pass === USERS.find((entry) => entry.userName === params.user).password) {
      req.user = USERS.find((entry) => entry.userName === params.user)
    }
  }
  next();
}
